...
slug: "processing-big-data-with-docker-in-the-cloud"
publishedAt: "2019-03-26"
author: "david-heyman"
authorName: "David Heyman"
summary: "Recently, we’ve been partnering with Hammerhead to design offline maps for their on-bike cycling computer. It’s been really interesting to work on a design project where size concerns (all the data…"
coverImage: "/images/blog/processing-big-data-with-docker-in-the-cloud-cover.png"
---
...
```
RUN apt-get update \  && apt-get install -y wget git zip software-properties-common default-jdk awscli
...
```
RUN wget https://bretth.dev.openstreetmap.org/osmosis-build/osmosis-latest.tgz \  && mkdir osmosis \
...
_Don’t forget:_ Each `RUN` command starts from the root of the virtual drive.
...
ENV S3_BUCKET="hammerhead-mapsforge" ENV AWS_ACCESS_KEY_ID="XXX"
...
```
d = digitalocean.Droplet(token=os.get('TOKEN'),
image='docker-18-04',
region='nyc2',
name=area_name,
...
active = []
for droplet in drops:  if droplet.status == 'off':    droplet.destroy()

 else:    active.append>(droplet.name)
```
